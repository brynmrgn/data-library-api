module PresentationHelpers
  ...
  def terms_with_link(terms, term_type)
    return "" unless terms
    
    # Normalize to array if it's a single Hash
    terms = [terms] if terms.is_a?(Hash)
    
    # Get the controller name for the current model (e.g., "research-briefings")
    ...
    links = terms.filter_map do |t|
      ...
      # Build URL like /research-briefings/subject/123456
      "<a href=\"/#{controller_name}/#{term_type}/#{term_id}\">#{label}</a>"
    end
